  ...
  def self.generate_report(params)
    ...
    when "po_by_date"
      ...
      contents.each do |content|
        content.purchase_order_details.each_with_index do |detail, i|
          sheet1.update_row row, i + 1, content.po_number, content.po_date.strftime("%d %B %Y"), content.supplier_full_name, detail.item_name, detail.price, detail.qty, detail.qty * detail.price
        end
      end
    ...
    when "receive_by_date"
      ...
      contents.each do |content|
        content.po_receive_details.each_with_index do |detail, i|
          sheet1.update_row row, i + 1, content.invoice_number, content.transaction_date.strftime("%d %B %Y"), content.supplier_full_name, detail.item_name, detail.price, detail.qty, detail.qty * detail.price
        end
      end
